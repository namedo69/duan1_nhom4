...
function getMostCommentProduct($limit)
{
    $sql = "SELECT p.product_id, p.name, p.image, p.price, COUNT(c.comment_id) AS comment_count FROM product AS p LEFT JOIN comment AS c ON p.product_id = c.product_id GROUP BY p.product_id, p.name, p.image, p.price ORDER BY comment_count DESC LIMIT $limit;";
